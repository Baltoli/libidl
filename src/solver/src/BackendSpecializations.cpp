#include <solver/BackendSpecializations.hpp>

template <unsigned... constants>
BackendConstantValue<constants...>::BackendConstantValue(
...
BackendLLVMSingle<Type>::BackendLLVMSingle(
    const FunctionWrap& wrap, std::function<bool(Type&)> pred)
    : BackendSingle(compute_hits(wrap, pred))
{
}

template <typename Type>
std::vector<unsigned> BackendLLVMSingle<Type>::compute_hits(
    const FunctionWrap& wrap, std::function<bool(Type&)> pred)
{
...
std::vector<std::string> function_whitelist
    = { "log", "exp", "sqrt", "cos", "sin", "tan", "logf", "expf", "sqrtf",
...
BackendFunctionAttribute::BackendFunctionAttribute(const FunctionWrap& wrap)
    : BackendLLVMSingle<llvm::Function>(wrap, [](llvm::Function& value) {
      ...
      if (std::find(function_whitelist.begin(), function_whitelist.end(),
              function_name)
...
BackendFloatZero::BackendFloatZero(const FunctionWrap& wrap)
    : BackendLLVMSingle<llvm::ConstantFP>(
          wrap, [](llvm::ConstantFP& value) { return value.isZero(); })
{
}

BackendIntZero::BackendIntZero(const FunctionWrap& wrap)
    : BackendLLVMSingle<llvm::ConstantInt>(
          wrap, [](llvm::ConstantInt& value) { return value.isZero(); })
{
}

template <unsigned op>
BackendOpcode<op>::BackendOpcode(const FunctionWrap& wrap)
    : BackendLLVMSingle<llvm::Instruction>(
          wrap, [](llvm::Instruction& inst) { return inst.getOpcode() == op; })
{
}

template <bool (llvm::Type::*predicate)() const>
BackendLLVMType<predicate>::BackendLLVMType(const FunctionWrap& wrap)
    : BackendLLVMSingle<llvm::Value>(wrap,
          [](llvm::Value& value) { return (value.getType()->*predicate)(); })
{
}

template <std::vector<std::vector<unsigned>> FunctionWrap::*forw_graph,
    std::vector<std::vector<unsigned>> FunctionWrap::*back_graph>
BackendLLVMEdge<forw_graph, back_graph>::BackendLLVMEdge(
    const FunctionWrap& wrap)
...
BackendLLVMDominate<inverted, unstrict, origin_calc,
    forw_graph>::BackendLLVMDominate(std::array<unsigned, 3> sizes,
    const FunctionWrap& wrap)
    : BackendDominate<inverted, unstrict>(
          { { (unsigned)std::get<0>(sizes) + (unsigned)get_origins(wrap).size(),
    ...
    , number_origins(get_origins(wrap).size())
{
  ...
  for (unsigned i = 0; i < number_origins; i++) {
    ...
    BackendDominate<inverted, unstrict>::template skip_invalid<0>(
        i, origins[i]);
...
template <bool inverted, bool unstrict, unsigned origin_calc,
    std::vector<std::vector<unsigned>> FunctionWrap::*forw_graph>
std::vector<unsigned>
BackendLLVMDominate<inverted, unstrict, origin_calc, forw_graph>::get_origins(
    const FunctionWrap& wrap)
{
    ...
    if ((data_sinks || data_origins) && llvm::isa<llvm::CallInst>(wrap[i])) {
      if (auto call_inst = llvm::dyn_cast<llvm::CallInst>(wrap[i])) {
        ...
        if (!(attributes.hasAttribute(
                  llvm::AttributeList::FunctionIndex, llvm::Attribute::ReadOnly)
                || attributes.hasAttribute(llvm::AttributeList::FunctionIndex,
