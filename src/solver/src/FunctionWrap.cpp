...
std::vector<std::vector<std::vector<unsigned>>> split_graph(
    const std::vector<std::vector<unsigned>>& graph)
{
...
std::vector<std::vector<std::vector<unsigned>>> split_reverse_graph(
    const std::vector<std::vector<unsigned>>& graph)
{
  std::vector<std::vector<std::vector<unsigned>>> result;
...
std::vector<std::vector<unsigned>> graph_union(
    const std::vector<std::vector<unsigned>>& graph1,
...
std::vector<std::vector<unsigned>> sort_graph(
    std::vector<std::vector<unsigned>> graph)
{
  for (unsigned i = 0; i < graph.size(); i++) {
    std::sort(graph[i].begin(), graph[i].end());
    graph[i].erase(
        std::unique(graph[i].begin(), graph[i].end()), graph[i].end());
  }

  return graph;
}

std::vector<std::vector<unsigned>> construct_cdg(
    ...
    std::unordered_map<llvm::Value*, unsigned>& value_hash)
{
  ...
  for (auto instruction : instr_hash) {
    if (llvm::TerminatorInst* terminator_instr
        = llvm::dyn_cast<llvm::TerminatorInst>(instruction.first)) {
      for (unsigned i = 0; i < terminator_instr->getNumOperands(); i++) {
        if (llvm::BasicBlock* target_block = llvm::dyn_cast<llvm::BasicBlock>(
                terminator_instr->getOperand(i))) {
...
std::vector<std::vector<unsigned>> construct_cfg(
    std::unordered_map<llvm::Instruction*, unsigned>& instr_hash,
    std::unordered_map<llvm::Value*, unsigned>& value_hash)
{
  ...
  for (auto instruction : instr_hash) {
    if (llvm::TerminatorInst* term_inst
        = llvm::dyn_cast<llvm::TerminatorInst>(instruction.first)) {
      for (auto& operand : term_inst->operands()) {
        if (llvm::BasicBlock* block_cast
...
std::vector<std::vector<unsigned>> construct_rdfg(
    std::unordered_map<llvm::Instruction*, unsigned>& instr_hash,
    std::unordered_map<llvm::Value*, unsigned>& value_hash)
{
  std::vector<std::vector<unsigned>> rdfg(
      value_hash.size() + instr_hash.size());

  for (auto instruction : instr_hash) {
    if (auto call_inst = llvm::dyn_cast<llvm::CallInst>(instruction.first)) {
      llvm::Instruction* instr
          = llvm::dyn_cast<llvm::Instruction>(call_inst->getCalledValue());
      ...
      if (it2 != instr_hash.end()) {
        rdfg[value_hash.size() + instruction.second].push_back(
            value_hash.size() + it2->second);
      }
