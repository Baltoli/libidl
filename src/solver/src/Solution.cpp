#include <solver/FunctionWrap.hpp>
    ...
    llvm::Function& function, unsigned max_solutions)
{
  ...
  std::transform(constraint.begin(), constraint.end(), labels.begin(),
      [](std::pair<std::string, std::unique_ptr<SolverAtom>>& v) {
  ...
  while (result.size() < max_solutions) {
    ...
    std::transform(solution.begin(), solution.end(),
        std::back_inserter(llvm_solution),
...
void Solution::set_precomputed_strings(
    std::shared_ptr<std::unordered_map<llvm::Value*, std::string>> strings)
{
...
Solution::Solution(
    std::vector<std::string> labels, std::vector<llvm::Value*> values)
    : single_value(nullptr)
{
    ...
    if (labels[i][0] == '[') {
      ...
      if (it != labels[i].end() && *it == ']') {
        if (it + 1 == labels[i].end()) {
          ...
          vector_parts[index].labels.emplace_back(
              std::string(it + 1, labels[i].cend()));
          vector_parts[index].values.emplace_back(values[i]);
        } else if (*(it + 1) == '.' && it + 2 != labels[i].end()
            && *(it + 2) != '[') {
          vector_parts[index].labels.emplace_back(
              std::string(it + 2, labels[i].cend()));
          vector_parts[index].values.emplace_back(values[i]);
        } else
          return;
      }
  ...
  if (vector_parts.empty()) {
    for (auto& part : map_parts)
      map_value.emplace_back(
          part.first, Solution(part.second.labels, part.second.values));
  }
}

Solution::operator llvm::Value*() const { return single_value; }

Solution Solution::operator[](std::string str) const
{
  auto find_it = std::find_if(map_value.begin(), map_value.end(),
      [&str](const std::pair<std::string, Solution>& x) {
        return x.first == str;
      });
...
Solution Solution::prune() const
{
  if (!map_value.empty() && vector_value.empty()) {
    ...
    for (auto& pair : map_value) {
      ...
      if (!pruned.map_value.empty() || !pruned.vector_value.empty()
          || pruned.single_value) {
...
std::string Solution::print_json(llvm::ModuleSlotTracker& slot_tracker) const
{
  ...
  if (single_value) {
    ...
    } else if (llvm::Function* func_value
        = llvm::dyn_cast<llvm::Function>(single_value)) {
      func_value->printAsOperand(out_stream, true, slot_tracker);
      instr_strings->insert({ func_value, string_value });
    } else if (llvm::Instruction* instr_value
        = llvm::dyn_cast<llvm::Instruction>(single_value)) {
    ...
    for (char c : out_stream.str()) {
      if (c == '\n')
        ...
        escaped_string += std::string(1, c);
    }

    while (escaped_string.size() > 0 && escaped_string[0] == ' ')
      escaped_string
          = std::string(escaped_string.begin() + 1, escaped_string.end());
