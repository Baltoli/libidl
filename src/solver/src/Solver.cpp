...
Solver::Solver(std::vector<std::unique_ptr<SolverAtom>> s)
    : iterator(UINT_MAX)
{
  swap_specials(std::move(s));
}

Solver::~Solver() { swap_specials(); }

std::vector<std::unique_ptr<SolverAtom>> Solver::swap_specials(
...
std::vector<SolverAtom::Value> Solver::next_solution(unsigned max_steps)
{
  ...
  while (max_iterations > 0) {
    ...
    SkipResult result
        = specializations[iterator]->skip_invalid(solution[iterator]);
